...
void *Rotate8(void* tid) {
    ...
    tn *= ip.Vpixels/NumThreads;
			...
			hp3=ip.Hpixels*3;
    for(row=tn; row<tn+ip.Vpixels/NumThreads; row++){
        memcpy((void *)Buffer, (void *) TheImage, (size_t) ip.Hbytes);
        col=0;
		cc=0.00;
		ss=0.00;
			Y=(double)v-(double)row;
        ...
        while(col<hp3){
			...
			if((NewCol>=0) && (NewRow>=0) && (NewCol<ip.Hpixels) && (NewRow<ip.Vpixels)){
				NewCol*=3;
				CopyImage[NewRow][NewCol]   = Buffer[row][col];
				CopyImage[NewRow][NewCol+1] = Buffer[row][col+1];
				CopyImage[NewRow][NewCol+2] = Buffer[row][col+2];
            }
...
void *Rotate9() {
	...
	memcpy((void *)Buffer, (void *)TheImage[nextRow], (size_t) ip.Hbytes);
